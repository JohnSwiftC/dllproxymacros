use proc_macro::{TokenStream, TokenTree, Ident, Delimiter, Group, Spacing, Punct};
use winapi::um::libloaderapi::{GetProcAddress, LoadLibraryA};
use std::ffi::CString;
use syn::{parse_macro_input, token::Token, FnArg, ItemFn, Pat, ReturnType, Type, PatIdent};
use quote::quote;
...
pub fn prehook(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let mut dllname = attr.next().expect("DLL Name must be specified as a literal.").to_string();
    let _ = attr.next().expect("Must be delimited by a comma.");
    let mut function_name = attr.next().expect("Function name must be specified as a literal.").to_string();
    ...
    let function_args = fn_args_as_params(TokenStream::from_iter(function_sig_stream.clone().into_iter()));
    ...
    new_stream.extend::<TokenStream>(quote!{
        #[unsafe(export_name=#function_name)]
    }.into());
    new_stream.extend::<TokenStream>(quote!{
        extern "system"
    }.into());
    new_stream.extend::<TokenStream>(TokenStream::from_iter(function_sig_stream.clone().into_iter()));
    ...
    new_body.extend::<TokenStream>(quote!{ // Execute proxy function
        let mut ret = func
    }.into());
    new_body.extend::<TokenStream>(TokenTree::Group(Group::new(Delimiter::Parenthesis, function_args)).into());
    ...
    unsafe_block.extend::<TokenStream>(quote!{
        unsafe
    }.into());
    unsafe_block.extend::<TokenStream>(TokenTree::Group(Group::new(Delimiter::Brace, new_body)).into());

    new_stream.extend::<TokenStream>(TokenTree::Group(Group::new(Delimiter::Brace, unsafe_block)).into());
...
fn fn_to_fn_ptr(input: TokenStream) -> TokenStream {
    ...
    let param_types = input_fn.sig.inputs.iter().filter_map(|arg| {
        match arg {
...
fn fn_args_as_params(input: TokenStream) -> TokenStream {
    ...
    let param_names = input_fn.sig.inputs.iter().filter_map(|arg| {
        match arg {
